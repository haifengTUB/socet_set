  ...
  if ($q)
    {
       ...
       if (($it eq "Real") && ($f) && (!($s))) {
         ...
         $args = ("/usgs/dev/contrib/bin/gdal_translate -of $of -co \"quality=$q\" -ot Int16 -a_srs \"ESRI::$outprj\" -a_nodata $NULL2 $outaux $output");
       } elsif ($s==8) {
         $args = ("/usgs/dev/contrib/bin/gdal_translate -of $of -co \"quality=$q\" -a_srs \"ESRI::$outprj\" -a_nodata $NULL1 $outaux $output");
       } elsif ($s==16) {
         $args = ("/usgs/dev/contrib/bin/gdal_translate -of $of -co \"quality=$q\" -a_srs \"ESRI::$outprj\" -a_nodata $NULL2 $outaux $output");
       } else {
         $args = ("/usgs/dev/contrib/bin/gdal_translate -of $of -co \"quality=$q\" -a_srs \"ESRI::$outprj\" -a_nodata $nodata $outaux $output");
       }
       print "submitting system command: $args\n";
       system($args) == 0
          or die "system $args failed: $?";
